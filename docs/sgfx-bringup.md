...
The [vendor-runlist candidate](gpu-runlist-0915.md) uses the Switch Linux vendor's
...
lines to retain their trailing values in captured logs.
...
The user physically tested this source. Hardware latches the exact physical
runlist base for `[0, 0]`, but channel zero remains pending with PBDMA invalid,
all reported error registers zero, and unchanged GET/reference/fence backing.
The complete terminal error is `FIFO host-method completion timeout`; evidence is in
[gpu-runlist-0915-verification.json](gpu-runlist-0915-verification.json).

The [scheduler-order candidate](gpu-scheduler-order-0915.md) now keeps channel
zero unbound during PFIFO setup, completes authenticated GR initialization,
then binds and executes both host proofs before graphics setup. This matches the
Linux application-visible ordering while preserving every real execution and

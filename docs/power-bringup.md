# Switch battery and input-power telemetry

The console BSP enables `scarlet-driver-switch-power`. It registers
`switch-battery` and, when detected, `switch-usb-input` with Scarlet's common
power-supply API. `power-info` reads the same API used by Scarlet Shell.

The driver binds the existing `maxim,max17050` node on Tegra I2C1, verifies the
gauge device ID and uses `maxim,rsns-microohm` from the selected device tree.
The adjacent BQ24193 is identified at address 0x6b. Failure to identify that
charger does not hide the independently working gauge.

Reads set only I2C register pointers. The driver does not write gauge models,
calibration, charge limits, watchdog configuration or USB-PD negotiation state.
The firmware's setup remains in effect. The latched fault register is not read.

- Gauge: presence, reported SOC, cell voltage, average signed battery current
  and temperature. A gauge POR suppresses SOC until its model is initialized.
- Charger: power-good excluding OTG output, charge phase and input-current limit.
  Negative average battery current is reported as discharging even on external
  power (battery supplementation). A plugged-in source alone is not charging.
- An I2C failure returns unavailable data; it never becomes a synthetic 0%.

## Sources

- [MAX17047/MAX17050 data sheet](https://www.analog.com/media/en/technical-documentation/data-sheets/MAX17047-MAX17050.pdf).
- [TI BQ24193 SLUSBG7A](https://www.ti.com/lit/ds/symlink/bq24193.pdf), registers 00, 08 and 0A.
- [Hekate MAX17050 board conversions](https://github.com/CTCaer/hekate/blob/e487de8fdd6ca9c3f608d1d18c097a86355912b9/bdk/power/max17050.c).
  The selected Switch DT supplies 10,000 microohms, accounting for the board's
  physical shunt and inherited current gain.

## Verification

Host tests exercise signed conversions, fractional SOC, POR, battery absence,
bus errors, input limits, charge termination, OTG and battery supplementation.
The normal console kernel, shell/power-info cross checks and full console
image build pass. The common ABI and shell state-selection tests pass as well
(31 host tests total, including eight new power tests). The Nix development
shell includes CMake and e2fsprogs for the full image. Its default cross-C
compiler follows Scarlet's `TARGET_CC` policy, allowing application-specific
compiler settings for packages such as yt.

### Erista hardware, 2026-09-21

Switchvisor USB boot returned two providers through `power-info`:

| Observation | First sample |
| --- | --- |
| Battery present | Yes |
| Reported capacity | 18.4% |
| Cell voltage | 3,590,000 uV |
| Average battery current | -496,406 uA (discharging) |
| Temperature | 39,367 mC |
| External input online | Yes |
| Configured input limit | 500,000 uA |

The later sample was 18.0%, 3,608,125 uV and -499,062 uA. This connection
provided external input while the battery still supplemented the system load;
it must use the plugged-in icon, not the charging icon.

The user confirmed the icon immediately right of volume and the capacity/power
details in Control Center. They also confirmed unplugging USB changed the icon
to one capacity bar, and reconnecting restored the plug icon. The shell and
`power-info` were copied to the SD
rootfs and their SHA-256 readbacks matched the packaged binaries. Audio remained
at zero. The kernel was loaded over USB; this does not claim a new standalone
SD kernel installation.

Evidence is in `.cache/power/validation/`: `unit.log`, `console-image.log`,
`uart.log` and `verification.json`. Cable transitions can be checked by
disconnecting/reconnecting input power, allowing one five-second polling
interval for each change. Percentage is shown in Control Center, not the bar.
